		...
		public static function get_row_element( $i, $m = [] ) {
			...
			if ( $elm === 'logo' || $elm === 'logo_2' ) {
				...
				if ( $logo ) {
					...
					$logo_html = '<div class="logo_is_img ' . esc_attr( $elm ) . '"><a href="' . self::$home_url . '" title="' . esc_html( get_bloginfo( 'description' ) ) . '"><img src="' . esc_url( $logo ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" width="' . esc_attr( $lw ) . '" height="' . esc_attr( $lh ) . '" style="width: ' . esc_attr( $lw . $lp ) . '"' . $escaped_size_on_sticky . '>' . do_shortcode( $slogan ) . '</a>';

				} else {

					$logo_html = '<div class="logo_is_text ' . esc_attr( $elm ) . '"><a href="' . self::$home_url . '" title="' . esc_html( get_bloginfo( 'description' ) ) . '"><h1>' . esc_html( get_bloginfo( 'name' ) ) . '</h1>' . do_shortcode( $slogan ) . '</a>';
				...
				$container = empty( $i['sk_shop_container'] ) ? '' : self::sk_inline_style( $i['sk_shop_container'] );
				$icon_style = empty( $i['sk_shop_icon'] ) ? '' : self::sk_inline_style( $i['sk_shop_icon'] );
				$icon = empty( $i['shopcart_icon'] ) ? ( $elm === 'wishlist' ? 'fa fa-heart-o' : 'fa fa-arrow-right-arrow-left' ) : $i['shopcart_icon'];
