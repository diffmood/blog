	...
	public function share() {
		...
		foreach( $share as $name ) {
			...
			if ( isset( $url[ $name ] ) ) {
				...
				echo '<a href="' . $url[ $name ]['url'] . '" rel="noopener noreferrer nofollow" class="cz-' . ( ( $name === 'twitter' ) ? 'x-' : '' ) . $name . '" ' . ( $tooltip ? 'data-' : '' ) . 'title="' . esc_attr( $title_prefix . ' ' . $url[ $name ]['title'] ) . '"' . $custom_data . '><i class="' . $ico . '"></i><span>' . esc_html( $url[ $name ]['title'] ) . '</span></a>';
